...
class Waiting_for_0_From_below(State):
    ...
    def on_event(self, event):
        if event == 0:
            ...
            received_file = open(client_received_file, "w")
            received_file.write(data.decode())  # deliver the data to the upper layer
            received_file.close()
...
class Waiting_for_1_From_below(State):
    ...
    def on_event(self, event):
        if event == 1:
            ...
            received_file = open(client_received_file, "a")
            received_file.write(data.decode())  # deliver the data to the upper layer
            received_file.close()
